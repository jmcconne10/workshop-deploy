...
1. **Git server** — a bare Git server on **Red Hat UBI** (`git` + `httpd` serving smart HTTP via `git-http-backend`), running as a container in the cluster
   - Deployment with a persistent volume for repo storage; **no database** (repos are plain files) and **no web UI**
...
3. **BuildConfigs** — OpenShift-native build automation (no separate CI/CD pipeline needed for POC)
   ...
   - A server-side **`post-receive` hook** on the git server fires the BuildConfig generic webhooks: push to `dev` rebuilds dev, merge to `main` rebuilds prod
   - ImageStreams for dev and prod images
4. **Helm Chart** — the whole environment (git server, repos, BuildConfigs, deployments, routes, services) deploys with a single `helm install`
   - The git server image is built in-cluster from a UBI `Containerfile`; the git server's startup script inits the bare repo, seeds the starter files, and installs the `post-receive` hook
...
- ~5 hackathon teams, each with their own git repo (main + dev branches)
...
- Set explicit, small resource requests/limits on every container (e.g., git server ~100m CPU / 128Mi memory request; Flask app ~50m CPU / 128Mi memory)
- **Single replica** for everything — no HA, no autoscaling
- Use the smallest viable persistent volume for the git server (1Gi is plenty for a POC repo)
- **No database** — the git server stores repos as plain files; do NOT deploy PostgreSQL/MySQL/SQLite
...
1. `helm install` deploys the git server + starter app + BuildConfigs into the sandbox project
2. The git server is reachable via an OpenShift Route; starter repo exists with `main` and `dev` branches
3. Pushing a code change to `dev` automatically triggers a rebuild and redeploy of the dev app
...
- Branch naming: `feature/<short-description>` (e.g., `feature/gitserver-deployment`, `fix/buildconfig-webhook`)
- One feature branch per logical unit of work. Keep branches small and focused.
- Commit early and often within a feature branch with clear, descriptive commit messages (imperative mood: "Add git server deployment template").
...
- Every configurable item (image tags, resource limits, git server settings, repo names, hostnames) belongs in `values.yaml` with sane sandbox-sized defaults.
- Add comments in templates explaining anything non-obvious (especially the git server startup script and the `post-receive` webhook wiring).
...
- Build ONLY the POC scope: one project, one git server, one starter repo, one Flask app with dev + prod deployments, BuildConfigs with webhooks.
